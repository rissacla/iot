...
static BLEUUID ledStatusCharacteristic ("01234567-0123-4567-89ab-0123456789fg");
...
// Callback function for handling notifications for the LED characteristic
class LEDCharacteristicCallbacks : public BLECharacteristicCallbacks {
    void onNotify(BLECharacteristic *pCharacteristic) {
        std::string value = pCharacteristic->getValue();
        if (value.length() > 0) {
            // Interpret the received value as a boolean
            ledStatus = value[0];
        }
    }
};
...
void setup() {
  ...
  // Retrieve the LED characteristic and assign callback function
  ledStatusCharacteristic = pRemoteService->getCharacteristic(ledStatusCharacteristicUUID);
  if (ledStatusCharacteristic != nullptr) {
      ledStatusCharacteristic->registerForNotify(new LEDCharacteristicCallbacks());
  }
}
...
void loop() {
  ...
  // Check if the button is pressed to read the LED status from the server
  if (digitalRead(M5_BUTTON_HOME) == LOW) {
      readLEDStatusFromServer();
  }
  delay(1000); // Delay one second between loops.
}

// Function to read the LED status from the server
void readLEDStatusFromServer() {
    // Check if the characteristic is available
    if (ledStatusCharacteristic != nullptr) {
        // Read the LED status from the characteristic
        std::string value = ledStatusCharacteristic->getValue();
        if (value.length() > 0) {
            // Interpret the received value as a boolean
            ledStatus = value[0];
        }
...
// Function to write to the LED characteristic and change the LED status on the server
void writeLEDStatusToServer(bool status) {
    // Check if the characteristic is available
    if (ledStatusCharacteristic != nullptr) {
        // Write the new LED status to the characteristic
        ledStatusCharacteristic->writeValue(status ? "1" : "0");
    }
}